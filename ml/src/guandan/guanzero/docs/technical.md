...
**Start/end values.** Literature default is start=1.0, end=0.01–0.05. GuanZero uses start=0.1 (conservative) because self-play + replay buffer already provides diversity without needing fully random early play.
...
GuanZero no longer truncates legal actions for policy reasons. Actors score the full
