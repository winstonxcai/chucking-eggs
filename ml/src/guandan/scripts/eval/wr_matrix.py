...
Usage:
    uv run python -m guandan.scripts.eval.wr_matrix --games 200
    uv run python -m guandan.scripts.eval.wr_matrix --games 200 --agents greedy,heuristic,strategic,jidan
    uv run python -m guandan.scripts.eval.wr_matrix --games 200 --exclude-agents noai,llm
    uv run python -m guandan.scripts.eval.wr_matrix --games 200 --workers 4
"""
...
from guandan.scripts.eval._eval_worker import play_n_games

DEFAULT_EXCLUDED_AGENTS = ("noai",)

MatchupResult = dict[str, Any]


def _parse_agent_list(values: list[str] | str | None) -> list[str]:
    if values is None:
        return []
    ...
    agents: list[str] = []
    for value in values:
        agents.extend(name.strip() for name in value.split(",") if name.strip())
    return list(dict.fromkeys(agents))


def _default_agents(excluded: list[str]) -> list[str]:
    excluded_set = set(excluded)
    return [name for name in AGENT_REGISTRY if name not in excluded_set]


def _validate_agent_names(agent_names: list[str], *, allow_dart: bool = False) -> None:
    allowed = set(AGENT_REGISTRY)
    if allow_dart:
        allowed.add("dart")
    unknown = sorted(set(agent_names) - allowed)
    if unknown:
        raise ValueError(
            f"unknown agent name(s): {unknown}. Known agents: {sorted(allowed)}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated agent names. Defaults to all registered rule-based "
            "agents minus --exclude-agents."
        ),
    )
    parser.add_argument(
        "--exclude-agents",
        ...
        help=(
            "Agents to exclude from the default registry-derived list. "
    ...
    parser.add_argument(
        "--output", type=str, default="ml/results/wr_matrix",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also overwrite guandan/elos.json for package/web defaults. "
            "By default ELOs are written only under --output."
        ),
    )
    args = parser.parse_args()

    excluded_agents = _parse_agent_list(args.exclude_agents)
    _validate_agent_names(excluded_agents)
    if args.agents:
        agent_names = _parse_agent_list(args.agents)
    else:
        agent_names = _default_agents(excluded_agents)
    _validate_agent_names(agent_names, allow_dart=bool(args.checkpoint))
    ...
    elo_path = out_dir / "elos.json"
    elo_path.write_text(json.dumps(elo_export, indent=2) + "\n")
    print(f"ELOs exported to {elo_path}")
    if args.export_package_elos:
        package_elo_path = Path(__file__).resolve().parents[2] / "elos.json"
        package_elo_path.write_text(json.dumps(elo_export, indent=2) + "\n")
        print(f"Package ELOs exported to {package_elo_path}")
