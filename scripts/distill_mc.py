...
def _train_batch_mse(decisions, q_net, optimizer, device, stats, role):
    ...
    Preserves value semantics: teaches ordering without destroying existing
    Q-value scale. Unlike cross-entropy, does not collapse non-expert actions.
    """
    N = len(decisions)
    ...
    # Expand to all candidates, single MLP pass
    ...
    # Accuracy: does Q-argmax match score-argmax?
...
def main() -> None:
    ...
    log.info("Loss: MSE(Q-values, MC scores)  [preserves value semantics]")
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(lead_decisions), desc=f"Epoch {epoch+1}/{args.epochs} [lead]",
            ...
            for i in range(0, len(lead_decisions), args.batch_size):
                ...
                if batch:
                    _train_batch_mse(batch, q_lead, opt_lead, device, stats, "lead")
        ...
        with tqdm(total=len(follow_decisions), desc=f"Epoch {epoch+1}/{args.epochs} [follow]",
            ...
            for i in range(0, len(follow_decisions), args.batch_size):
                ...
                if batch:
                    _train_batch_mse(batch, q_follow, opt_follow, device, stats, "follow")
                pbar.update(len(batch))
