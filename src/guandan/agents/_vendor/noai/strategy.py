...
# Changes: import paths, de-singleton (removed module-level `Strategy = Strategy()`),
...
class Strategy(object):
    ...
    def UpdateRVwhenOppoPlaysSmall(self):
        if (self.greaterPos != (self.myPos + 1) % 4 and self.greaterPos != (self.myPos + 3) % 4):
            ...
            oppoHandValue = CountValue().ActionValue(self.curAction[2], self.curAction[0], self.curAction[1],
                                                                self.curRank)
            part = (self.myPos + 2) % 4
