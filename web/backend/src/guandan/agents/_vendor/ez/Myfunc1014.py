    ...
    def firstcard(self,hd, cur):
        ...
        for i in hdcopy:
            ...
            if i.find(cur) == 1 and i[0] == 'H':
                dict['numh'].append(i)
        # pass  #print(len(self.getcard(hdcopy, dict['numh'])['Straight'][1]))
        while len(self.getcard(hdcopy, dict['numh'])['Straight']) > 0:
            # pass  #print("长度",len(getcard(hdcopy))['Straight'][1::2])
            ...
            for i in dict['ThreePair'][1::2]:
                for j in i:
                    if poke.index(j[1])<=10:
                        ...
                        hdcopy.remove(j)
       # pass  #print("这是hdcopy", hdcopy)
        pass  #print("这是dict",dict)
        dict['Single']=[]
        dict['Pair']=[]
        dict['Trips']=[]
        pass  #print("这是先手配的牌",dict)
        return (hdcopy, dict)
                       ...
                       # pass  #print("这是eb", eb, num)
                        for i in eb:
                            if single.count(i) == 1:
                                ...
                                # pass  #print(dict['Pair'][dict['Pair'].index(i)+1][0])
                                num -= 1
                               # pass  #print("这是eb", eb, num)
                            elif trip.count(i) == 1:
...
# gp=msg['greaterPos']
# act=data['actionList']
# pass  #print(msg['curPos'])
# pass  #print(lasthand.firstcard(hd,'2')[0])
# pass  #print(lasthand.firstcard(hd,'2')[1])
# pass  #print("二手配完牌",lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]))
# # pass  #print("寻找索引",lasthand.getindex(act,lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]),'2')))
# pass  #print("这是手牌",lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]))
# pass  #print("index",lasthand.getindex(act,lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,msg['curRank'])[0],lasthand.firstcard(hd,msg['curRank'])[1]),'2')))
# pass  #print("这是要出的牌",lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,msg['curRank'])[0],lasthand.firstcard(hd,msg['curRank'])[1]),'2'))
# pass  #print("拿到索引",lasthand.getindex(act,['S3', 'C3']))
